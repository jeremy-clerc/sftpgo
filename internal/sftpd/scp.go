...
func (c *scpCommand) handleUploadFile(fs vfs.Fs, resolvedPath, filePath string, sizeToRead int64, isNewFile bool, fileSize int64, requestPath string) error {
	...
	checks := c.connection.GetCreateChecks(requestPath, isNewFile, false) | vfs.CheckStreamWrite
	file, w, cancelFn, err := fs.Create(filePath, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, checks)
	if err != nil {
